...
public class PullAbility extends ItemAbility {
    ...
    public void execute(@NotNull AbilityExecuteContext context) {
        Player player = context.playerData().getSender().get();
        player.setVelocity(player.getVelocity().setY(0).add(player.getLocation().getDirection().multiply(2).add(new Vector(0, 0.75, 0))));
